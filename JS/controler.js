...
fetch('/book', {
    ...
    .then(result => {
        ...
        result.forEach( result => {
            ...
            finish.innerHTML = `<button class="ler-button" onclick="finishConsult('${result.emailorname}')">Finalizar.</button>`;
        });
}).catch(error => console.error(error));
